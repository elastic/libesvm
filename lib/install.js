...
module.exports = function (options, cb) {
  ...
  if (version) {
    install = function (dest) {
      return installVersion({ dest: dest, version: version, log: log }, cb); 
    };
  } 
  else if (branch) {
    install = function (dest) {
      return installFromBranch({ dest: dest, branch: branch, log: log }, cb); 
    };
  }
  else if (binary) {
    install = function (dest) {
      return installFromBinary({ dest: dest, binary: binary, log: log }, cb); 
    };
  } else {
